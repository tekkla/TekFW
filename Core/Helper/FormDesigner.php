...
final class FormDesigner extends Form
{
    ...
    public function &createElement($type, $name = null, $params = null)
    {
        switch ($type) {
            ...
            case 'date':
            case 'date-iso':
                $element = DateTimePicker::factory($name)->setFormat('YYYY-MM-DD')
                    ->setMask('9999-99-99')
                    ->setMaxlenght(10);
                break;

            case 'date-us':
                $element = DateTimePicker::factory($name)->setFormat('mm/dd/yyyy')
                    ->setMask('99/99/9999')
                    ->setSize(10)
                    ->setMaxlenght(10);
                break;
            ...
            case 'date-gr':
                $element = DateTimePicker::factory($name)->setFormat('dd.mm.yyyy')
                    ->setMask('99.99.9999')
                    ->setSize(10)
                    ->setMaxlenght(10);
                break;
            ...
            case 'time-24':
                $element = DateTimePicker::factory($name)->setFormat('HH:mm')
                    ->setMask('99:99')
                    ->setSize(5)
                    ->setMaxlenght(5);
                break;
            ...
            case 'time-24s':
                $element = DateTimePicker::factory($name)->setFormat('HH:mm:ss')
                    ->setMask('99:99:99')
                    ->setSize(8)
                    ->setMaxlenght(8);
                break;
            ...
            case 'time-12':
                $element = DateTimePicker::factory($name)->setFormat('hh:mm A/PM')
                    ->setMask('99:99')
                    ->setSize(5)
                    ->setMaxlenght(5);
                break;
            ...
            case 'time-12s':
                $element = DateTimePicker::factory($name)->showMeridian(true)
                    ->setFormat('hh:mm::ss A/PM')
                    ->setMask('99:99:99')
                    ->setSize(8)
                    ->setMaxlenght(8);
                break;
            ...
            case 'datetime':
            case 'datetime-iso':
                $element = DateTimePicker::factory($name)->setFormat('YYYY-MM-DD HH:mm')
            ...
            case 'multiselect' :
				/* @var Select $element */
				$element = Select::factory($name)->isMultiple(1)
                    ->setSize(10)
                    ->addCss('form-multiselect');
                break;
            ...
            case 'submit':
                $element = Button::factory($name)->setType('submit')
                    ->setInner($this->txt('btn_save'))
                    ->useIcon('save')
                    ->isPrimary();
                break;

            case 'reset':
                $element = Button::factory($name)->setType('reset')->setInner($this->txt('btn_reset'));
                break;
            ...
            case 'range':
                ...
                $element = Input::factory($name, 'number')->addCss('form-number')
                    ->addAttribute('min', $params[0])
                    ->addAttribute('max', $params[1]);
        ...
        foreach ($this->controls as $control_field => $control) {
            ...
            elseif ($control instanceof FormElementAbstract) {
                ...
                if ($control->hasLabel() && ! $control->getLabel()) {
                    $label = $this->txt($this->model_name . '_' . $this->uncamelizeString($control_field), $this->app_name);
                ...
                if ($control instanceof Input && $control->getType() == 'file') {
                    $max_size_field = Input::factory('MAX_FILE_SIZE')->setType('hidden')->setValue($this->di['core.io.file']->getMaximumFileUploadSize());
                ...
                if ($control->hasCompare()) {
                    ...
                    $compare_control = Input::factory($compare_name)->setType('hidden')
                        ->setValue($control->getCompare())
                        ->setId($control->getId() . '_compare');
