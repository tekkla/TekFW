...
final class Css
{
    ...
    public function init()
    {
        ...
        $version = $this->cfg->get('Core', 'style.bootstrap_version');
        
        // Core and theme file
        $file = '/' . $this->cfg->get('Core', 'style.theme') . '/css/bootstrap.min.css';
        
        // Add existing local user/theme related bootstrap file or load it from cdn
        if ($this->cfg->get('Core', 'style.bootstrap_use_local') && file_exists(THEMESDIR . $file)) {
            $this->link(THEMESURL . $file);
        } else {
            // Add bootstrap main css file from cdn
            $this->link('https://maxcdn.bootstrapcdn.com/bootstrap/' . $this->cfg->get('Core', 'style.bootstrap_version') . '/css/bootstrap.min.css');
        }
        
        // Fontawesome file
        $file = '/' . $this->cfg->get('Core', 'style.theme') . '/css/font-awesome.min.css';
        
        // Add existing font-awesome font icon css file or load it from cdn
        if ($this->cfg->get('Core', 'style.fontawesome_use_local') && file_exists(THEMESDIR . $file)) {
            $this->link(THEMESURL . $file);
        } else {
            $this->link('https://maxcdn.bootstrapcdn.com/font-awesome/' . $this->cfg->get('Core', 'style.fontawesome_version') . '/css/font-awesome.min.css');
        }
        
        // Add general TekFW css file
        $file = '/' . $this->cfg->get('Core', 'style.theme') . '/css/Core.css';
    ...
    public function getFiles()
    {
        ...
        if ($local_files) {
            ...
            $cache_object->setTTL($this->cfg->get('Core', 'cache.ttl_css'));
