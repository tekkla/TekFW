...
final class FormDesigner extends Form
{
	...
	public function &createElement($type, $name = '', $params = [])
	{
		...
		if (isset($this->model) && isset($this->model[$name]) && method_exists($element, 'setField')) {
	...
	public function build()
	{
		...
		foreach ($this->controls as $control_field => $control) {
			...
			elseif ($control instanceof FormElementAbstract) {
				...
				if ($control->isBound() && isset($this->model[$field_name])) {

					if (method_exists($control, 'setValue')) {
						$control->setValue($this->model[$field_name]);
					}

					if (method_exists($control, 'setInner')) {
						$control->setInner($this->model[$field_name]);
					}
					...
					if ($this->model[$field_name] !== '') {
						...
						if (method_exists($control, 'setSelectedValue')) {
							$control->setSelectedValue($this->model[$field_name]);
						}

						if ($type == 'checkbox' && $this->model[$field_name] == $control->getValue()) {
							$control->isChecked(1);
						}

						if (method_exists($control, 'switchOn') && $this->model[$field_name] = 1) {
							$control->switchOn();
						}
					}
				}
