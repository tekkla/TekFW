...
use Core\Lib\Content\Html\FormControlAbstract;
...
final class FormDesigner extends Form
{
    ...
    {
        return $this->createControl($type, $name, $params);
    }
     ...
     * @return HtmlAbstract
     */
    public function &createControl($type, $name = '', $params = [])
    {
        // Default element
        $element = [
            'class' => 'Form\Input'
        ...
        if ($element instanceof FormControlAbstract) {
    ...
    public function build()
    {
        ...
        foreach ($this->buttons as $btn => $text) {
            ...
            $button = $this->createControl('button', $btn_name);
            $button->setId($btn_id);
        ...
        foreach ($this->controls as $control_field => $control) {
            ...
            elseif ($control instanceof FormControlAbstract) {
                ...
                $field_name = $control->isBound() ? $control->getField() : $control->getName();
                ...
                elseif ($control instanceof Checkbox) {
                    ...
                    if (empty($label)) {
                        $label = $this->txt($this->control_name . '_' . $this->uncamelizeString($control_field), $this->app_name);
                    }
